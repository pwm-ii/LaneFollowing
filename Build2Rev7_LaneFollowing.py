...
try:
    while True:
        ...
        current_time = time.ticks_ms()  # Get current time
        blobs_found = False  # Flag to check if any blob is detected
        ...
        for i, roi in enumerate(ROIS, start=1):
            ...
            if filtered_blobs:
                blobs_found = True  # Blobs detected in this frame
        ...
        # Handle loss of blob detection
        if blobs_found:
            last_blob_time = current_time
        else:
            elapsed = time.ticks_diff(current_time, last_blob_time) / 1000  # ms to seconds
            if elapsed > 2:
                print("No blobs detected for 6 seconds. Stopping motor and resetting servo.")
                p0.duty_ns(0)
